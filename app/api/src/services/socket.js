...
module.exports = function socket( socket ) {
	...
	conn.on( 'ready', function connOnReady() {
		console.log(
			'WebSSH2 Login: user=' + socket.request.session.username + ' from=' + socket.handshake.address + ' host=' +
			socket.request.session.ssh.host + ' port=' + socket.request.session.ssh.port + ' sessionID=' +
			socket.request.sessionID + '/' + socket.id + ' mrhsession=' + socket.request.session.ssh.mrhsession +
		...
		socket.emit( 'footer',
			'ssh://' + socket.request.session.username + '@' +
			...
			function connShell( err, stream ) {
				...
				socket.on( 'data', function socketOnData( data ) {
					...
					if ( socket.request.session.ssh.serverlog.client ) {
						if ( data === '\r' ) {
							console.log(
								'serverlog.client: ' + socket.request.session.id + '/' + socket.id + ' host: ' +
								socket.request.session.ssh.host + ' command: ' + dataBuffer );
	...
	conn.on( 'keyboard-interactive',
		function connOnKeyboardInteractive( name, instructions, instructionsLang, prompts, finish ) {
			debugWebSSH2( 'conn.on(\'keyboard-interactive\')' );
			finish( [ socket.request.session.userpassword ] );
		} );
	if ( socket.request.session.username &&
		( socket.request.session.userpassword || socket.request.session.privatekey ) && socket.request.session.ssh ) {
	...
	function SSHerror( myFunc, err ) {
		...
		if ( socket.request.session ) {
			...
			if ( err && ( err.level === 'client-authentication' ) ) {
				...
				console.log(
					'WebSSH2 Logout: user=' + socket.request.session.username + ' from=' + socket.handshake.address +
					' host=' + socket.request.session.ssh.host + ' port=' + socket.request.session.ssh.port +
					' sessionID=' + socket.request.sessionID + '/' + socket.id + ' allowreplay=' +
					socket.request.session.ssh.allowreplay + ' term=' + socket.request.session.ssh.term );
				if ( err ) {
